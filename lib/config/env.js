 ...
 *
 * Next.js loads local `.env` files itself before any module here runs. On
 * Vercel, variables configured in Project Settings are injected directly into
 * `process.env`; no environment file is read in the deployed function.
 *
...
if (DATABASE_URL && !/^postgres(?:ql)?:\/\//i.test(DATABASE_URL)) {
  missing.push('DATABASE_URL (must start with postgres:// or postgresql://)');
}

const discreteDb = {
  host: read('SUPABASE_DB_HOST'),
  port: readPositiveInteger('SUPABASE_DB_PORT', 5432, { max: 65535 }),
