...
const sharedOptions = {
  dialect: 'postgres',
  // Sequelize normally requires `pg` dynamically from the dialect name. Next's
  // serverless file tracer cannot see that dynamic require, so Vercel omitted
  // the driver from the function bundle. Supplying the imported module makes
  // the dependency explicit and guarantees it is traced into the deployment.
  dialectModule: pg,
  logging: false,
  dialectOptions: {
