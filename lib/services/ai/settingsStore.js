...
import { clearModelCache } from './modelCatalog.js';
...
export const resolveActiveProvider = async () => {
  ...
  for (const row of rows.values()) {
    if (row.isActive && PROVIDERS.includes(row.provider) && (!env.isProduction || row.provider !== 'mock')) {
      return row.provider;
    }
  }
  // Mock replies must never masquerade as live AI in production, even if a
  // stale database row or environment variable selected it.
  return PROVIDERS.includes(env.AI_PROVIDER) && (!env.isProduction || env.AI_PROVIDER !== 'mock')
    ? env.AI_PROVIDER
    : 'gemini';
};
