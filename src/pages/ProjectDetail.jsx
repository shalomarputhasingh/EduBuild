...
const Section = ({ id, title, children, className = '' }) => (
  <section id={id} className={`rounded-card border border-slate-200 bg-white p-6 shadow-card ${className}`}>
    ...
    <div className="container-page py-8">
      ...
      {project.image && (
        <img
          src={project.image}
          alt=""
          className="mt-8 max-h-[420px] w-full rounded-card border border-slate-200 object-cover"
          onError={(event) => {
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={fact.label} className="rounded-card border border-slate-200 bg-white p-4 shadow-card">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-subtle">{fact.label}</p>
            <p className="mt-1 text-lg font-bold text-ink">{fact.value}</p>
          </div>
        ))}
      </div>
